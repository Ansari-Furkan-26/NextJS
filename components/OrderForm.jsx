...
const OrderForm = ({ language = 'english' }) => {
  ...
  const [isHydrated, setIsHydrated] = useState(false); // To handle hydration issues
  ...
  // Hydration fix: Ensures client-side rendering
  useEffect(() => {
    setIsHydrated(true);
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    router.push({
      pathname: '/checkout',
      query: { formData: JSON.stringify(formData) }, // Pass formData as query params
    });
  };
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100" id="orderform">
      <div className="max-w-6xl w-full bg-white rounded-lg shadow-xl flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/2 p-8">
          <h2 className="text-2xl font-semibold mb-6">{translations[language].title}</h2>
        ...
        <div className="w-full md:w-1/2 p-8 bg-gray-100 relative hidden md:block">
          <div
            ...
            style={{
              backgroundImage:
                'url("https://i.pinimg.com/736x/3f/1f/a7/3f1fa78019ce12434c8b59668fc2acd4.jpg")',
            }}
          >
